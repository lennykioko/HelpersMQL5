...
bool GetSMAValue(index = 0, int maDuration = 20, ENUM_TIMEFRAMES period = PERIOD_CURRENT, string symbol = NULL, bool verbose = false) {
    if(symbol == NULL) symbol = _Symbol;
    double maArray[];
    // int ma20Handle = iMA(_Symbol, PERIOD_CURRENT, 20, 0, MODE_SMA, PRICE_CLOSE);
    int maHandle = iMA(symbol, period, maDuration, 0, MODE_SMA, PRICE_CLOSE);
    ...
    if(verbose) {
        Print("MA value: " +  DoubleToString(maArray[index]));
    }

    return maArray[index];
}
